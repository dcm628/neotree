...
// sync. Local time comes from a POSIX time-zone rule the app sets
// (TIME_ZONE), stored in flash, which carries the daylight-saving changes
// (neotree/civil_time.hpp).
...
struct clock_status_t
{
    ...
    char tz[64];               // the POSIX rule in use ("UTC0" until one is set)
};
...
// changed. False if it isn't a valid rule. core0 (writes flash).
