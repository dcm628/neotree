...
here to cut that down without touching capture quality):
  - Freshness is measured, not predicted: neocam.drain_until_live
    watches each camera's own read latency and stops draining it once
    it directly observes the live-frame signature, instead of trusting
    a precomputed dwell formula. A formula-based dwell (compute_min_dwell_s,
    still used by the standalone diagnostic scripts) was confirmed by
    testing to sometimes make results dramatically worse in confusing,
    not-fully-understood ways even when the formula change itself was
    logically sound - e.g. scaling dwell up for a 4-camera round-robin
    tanked solve rates from ~100/1000 to ~1-10/1000. --max-dwell is a
    safety cap on drain_until_live, not a target wait time.
...
def compute_min_dwell_s(exposure_v4l2_units, fps, num_cameras=1,
                         led_refresh_margin_s=LED_REFRESH_MARGIN_S, frame_margin_s=0.025):
    """
    ...
    an LED, or turning everything off for a background frame):
      ...
      - max(exposure, 1/fps) * num_cameras: the frame we capture needs a
        full exposure window entirely *after* the change, or it
        partially blends the old and new state - but a UVC camera can
        never deliver a frame faster than its own negotiated frame
        period regardless of exposure (confirmed by testing: it
        silently clamps exposure down to fit 1/fps if you request more,
        so the frame *period* is the real floor, not the exposure value
        alone). neocam.drain_for round-robins ALL active cameras from a
        single thread, so the dwell has to cover every camera getting
        at least one live frame, not just one - confirmed by testing
        that using a single camera's frame period as the dwell with 4
        cameras round-robining at fps=10 (100ms/frame) meant most
        cameras never reached a live frame within the budget at all
        (catastrophic near-zero detection). At fps=30 this was masked
        because 4x a ~31ms live-read cost still comfortably fit, but the
        formula was still wrong in principle.
    ...
    frame_period_s = num_cameras * (1.0 / fps)
...
def capture_centroid_with_retry(cap, background, retries, threshold_value, retry_delay_s,
                                 retry_on_zero_blobs, retry_on_ambiguous, average_samples=1):
    """
    ...
    average_samples > 1: once a clean single-blob detection is found,
    takes that many additional consecutive reads (the camera is already
    confirmed live at this point, so no further draining is needed
    between them - each is a genuinely fresh frame roughly one frame
    period apart) and averages every clean detection among them. This
    targets a DIFFERENT problem than the retry logic above: even a
    reliably-detected LED's single-frame centroid carries real per-frame
    noise (confirmed by testing - tilt-fit residuals stayed elevated
    even after fixing a separate reliability bug that was causing missed
    detections entirely), and averaging several genuinely-fresh
    independent samples directly reduces that, regardless of its root
    cause (compression noise, residual exposure jitter, etc.).
    """
    ...
    for attempt in range(1, retries + 1):
        ...
        if frame is not None:
            ...
            if cx is not None:
                cxs, cys, areas = [cx], [cy], [blob_area]
                for _ in range(average_samples - 1):
                    extra_frame = neocam.capture_frame(cap)
                    if extra_frame is None:
                        continue
                    ecx, ecy, ecount, earea = neocam.find_single_blob_centroid(
                        extra_frame, background=background, threshold_value=threshold_value)
                    if ecx is not None:
                        cxs.append(ecx)
                        cys.append(ecy)
                        areas.append(earea)
                n = len(cxs)
                return sum(cxs) / n, sum(cys) / n, blob_count, sum(areas) / n, attempt
...
def run_sweep(conn, sweep_id, pylons, led_positions, max_dwell_s, max_background_dwell_s,
              retries, pixel_sigma_px, threshold_value, retry_delay_s,
              retry_on_zero_blobs, retry_on_ambiguous, threaded=False, average_samples=1):
    ...
    # connection trouble.
    #
    # Freshness is now measured, not predicted: a fixed precomputed
    # dwell time (drain_for) was confirmed by testing to be fragile in
    # confusing, not-fully-understood ways - e.g. scaling dwell up for a
    # 4-camera round-robin was logically justified but tanked solve
    # rates from ~100/1000 to ~1-10/1000. drain_until_live watches each
    # camera's own read latency and stops draining it once it directly
    # observes the live-frame signature, instead of trusting a formula.
    # max_dwell_s/max_background_dwell_s are now safety caps (give up
    # and move on if a camera never goes live), not target times - the
    # common case exits well before the cap.
    ...
    try:
        for walked, i in enumerate(led_positions, start=1):
            ...
            time.sleep(LED_REFRESH_MARGIN_S)
            neocam.drain_until_live(all_caps, max_duration_s=max_background_dwell_s)
            if threaded:
            ...
            time.sleep(LED_REFRESH_MARGIN_S)
            neocam.drain_until_live(all_caps, max_duration_s=max_dwell_s)
            ...
            if threaded:
                ...
                for pylon in pylons:
                    ...
                    lit_futures[pylon["pylon_id"]] = executor.submit(
                        ...
                        retry_delay_s, retry_on_zero_blobs, retry_on_ambiguous, average_samples)
                for pylon in pylons:
...
def main():
    ...
    parser.add_argument('--max-dwell', type=float, default=1.0,
                         help="safety cap (seconds) for drain_until_live - NOT a target wait time, "
                              "it exits as soon as each camera's reads directly show the live-frame "
                              "latency signature (see neocam.drain_until_live); this only bounds the "
                              "worst case if a camera never goes live at all")
    parser.add_argument('--dwell', type=float, default=None,
                         help="override --max-dwell for the post-LED-on capture specifically")
    parser.add_argument('--background-dwell', type=float, default=None,
                         help="override --max-dwell for the all-off background capture specifically")
    ...
    parser.add_argument('--average-samples', type=int, default=3,
                         help="once a clean single-blob detection is found, average this many "
                              "consecutive live frames' centroids together instead of trusting one "
                              "frame - targets per-frame centroid noise directly (tilt-fit residuals "
                              "stayed elevated even after fixing a separate reliability bug), at the "
                              "cost of (average_samples-1) extra live-frame periods per detection")
    ...
    try:
        ...
        # read-latency signature (see neocam.drain_until_live), so these
        # only bound the worst case (a camera that never goes live at
        # all) rather than setting how long a normal LED actually waits.
        # A precomputed formula-based dwell (even one that looked
        # logically sound) was confirmed by testing to sometimes make
        # results dramatically worse in confusing ways - measuring
        # freshness directly sidesteps needing that formula to be right.
        dwell_s = args.dwell if args.dwell is not None else args.max_dwell
        background_dwell_s = args.background_dwell if args.background_dwell is not None else args.max_dwell
        print(f"Using drain_until_live with a {dwell_s:.2f}s safety cap "
              f"(exits early once each camera's reads show the live-frame signature)")
